...
const Cargo = () => {
	...
	const init = async () => {
		const { data } = await ApiRequest().get('/cargo')
	...
	const onDelete = async (id) => {
		try {
			const { data } = await ApiRequest().post('/cargo/eliminar', { id: id })
			setMensaje({
	...
	const onSubmit = async () => {
		try {
			const { data } = await ApiRequest().post('/cargo/guardar', body)
			handleDialog()
	...
	const onEdit = async () => {
		try {
			const { data } = await ApiRequest().post('/cargo/editar', body)
			handleDialog()
			setBody(initialState)
			setMensaje({
				ident: new Date().getTime(),
		...
		<>
			...
			<Page title="Cafesoft | Cargos">
				...
				<Container maxWidth='lg'>
					<Box sx={{ pb: 1 }}>
						<Typography variant="h5" className="titulo">Cargos</Typography>
					</Box>
					<Grid container spacing={2}>
